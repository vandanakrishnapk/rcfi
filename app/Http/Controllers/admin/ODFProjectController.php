...
class ODFProjectController extends Controller
{
    ...
    public function getodfProjectData(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = DB::table('odf_tables')
                ...
                ->where('odf_tables.project_id', 'like', '%OC%')  // Only orphan care projects (project_id contains 'OC')
                ->get();
    ...
    {        
        $project = DB::table('odf_tables')
            ...
            ->leftjoin('odf_funds','odf_funds.project_id','=','odf_tables.id')
            ->select('odf_tables.*','donors.partner_name as donorName','markaz_orphan_cares.applicationId','odf_funds*')
