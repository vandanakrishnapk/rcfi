...
class diffProjectController extends Controller
{
    public function dodiffProject(Request $request)
    {
        // Validation rules
...
public function getdiffProjectData(Request $request)
{
    if ($request->ajax()) {
        ...
        $data = DB::table('odf_tables')
            ...
            ->leftjoin('differently_abled', 'odf_tables.application_id', '=', 'differently_abled.diffId')
            ->select('odf_tables.*', 'donors.partner_name as donorName', 'differently_abled.applicationId', 'odf_funds.amount', 'odf_funds.current')
            ->where('odf_tables.status', '!=', 1)  // Exclude records where status = 1
            ->where('odf_tables.project_id', 'like', '%DA%')
            ->get(); 
    ...
    public function projectdiffViewMore($id) 
    {        
        $project = DB::table('odf_tables')
            ->leftjoin('donors','donors.donor_id','=','odf_tables.donor_name')
            ->leftjoin('differently_abled','odf_tables.application_id','=','differently_abled.diffId')
            ->select('odf_tables.*','donors.partner_name as donorName','differently_abled.applicationId')
    ...
    public function editdiffProject($id)
    {
        $project = DB::table('odf_tables')
        ->leftjoin('donors','donors.donor_id','=','odf_tables.donor_name')
        ->leftjoin('differently_abled','odf_tables.application_id','=','differently_abled.diffId')
    ...
    public function updatediffProject(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'agency_id'          => 'required|string',
            'donor_name'         => 'required|exists:donors,donor_id',
            'application_id'     => 'required|exists:differently_abled,diffId',
            'account_name'       => 'required|string',
    ...
    public function deletediffProject($id)
    {
    ...
    public function adddiffFund(Request $request)
    {
...
public function getdiffCurrent()
{
    return response()->json([
        'message' =>'success'
    ]);
...
public function updatediffStatus($id)
{
        try {
            // Find the record by ID
            $project = odf_table::findOrFail($id);
...
public function updatediffpaymentStatus($id)
{
        try {
            // Find the record by ID
            $project = odf_table::findOrFail($id);    
