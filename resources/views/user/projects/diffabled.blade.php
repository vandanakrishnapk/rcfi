                            ...
                            <div class="modal-body p-4">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" name="application_id">
                                       ...
                                       <option value="{{ $opcares->diffId }}">{{ $opcares->applicationId }}</option>
                                       @endforeach
                                    </select>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p id="modalMessage"></p>
                        <p>Agency Project Number: <span id="modaldeleteUserName"></span></p>
                    </div>
                    ...
                    <div class="card-header">
                        <div class="row">
                            ...
                            <div class="col-12">
                                ...
                                <h4 class="widgetcolor p-3 text-center rounded fw-bold border border-success" style="color:white;">Differentlyy Abled</h4>
                    ...
                    <div class="card-header">
                        <div class="row">
                            ...
                            <div class="col-12">
                                ...
                                <h4 class="widgetcolor p-3 text-center rounded fw-bold border border-success" style="color:white;">Differently Abled</h4>
...
<div id="paidConfirmationModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="deleteConfirmationModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header widgetcolor">
                <h5 class="modal-title" id="deleteConfirmationModalLabel">Confirm Completion</h5>
            ...
            <div class="modal-body">
                <p id="modalpaidMessage"></p>
                <p>Project ID: <span id="modalpaidUserName"></span></p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn pro cancel" data-dismiss="modal">Cancel</button>
                <button type="button" id="paid" class="btn btn-danger">Paid</button>
            </div>
        </div>
    </div>
</div> 
...
$(document).ready(function() {
    ...
    $('#projectTable').DataTable({
        ...
        dom: 'Bfrtlip',
            ...
            ajax: {
                ...
                url: `{{ url('/user/projects/diff/datatable') }}`,
                type: 'POST',
                dataSrc: function(json) {
               console.log(json); // This will log the entire JSON response from the server
        return json.data; // Ensure the correct data structure is returned
    },
    ...
    render: function(data, type, row, meta) {
     ...
     <i class="bi bi-cash-stack"></i>
            ...
            <button type="button" class="btn btn-dark  btn-sm mb-2 float-end paid ms-2" data-id="${row.id}" data-pro="${row.project_id}" title="Paid">
            Paid
            </button> 
...
$(document).ready(function() {
    $(".submit-project").click(function(e) {
        ...
        $.ajax({
            url: `{{ url('/user/projects/diff/new') }}`,
            type: "POST",
            data:data,       
            dataType: "JSON",
            processData: false,
...
$(document).on('click', '.view-more', function() {
    ...
    if (ProjectId !== undefined) {
        $.get(`{{ url('/user/projects/diff/view/more') }}/${ProjectId}`, function(response) {
...
$('#projectTable').on('click', '.edit', function() {
    ...
    $.get(`{{ url('/user/projects/diff/edit') }}/${projectId}`, function(data) {
...
$(document).ready(function() {
    $('#EditprojectForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            url: "{{ route('user.updatediffProject') }}",
            data: formData,
            dataType: 'json',
            beforeSend: function() {
...
$(document).on('click', '.delete', function() {
    ...
    $('#modaldeleteUserName').text(userName);
    ...
    $('#confirmDelete').off('click').on('click', function() {
        $.ajax({
            url: `{{ url('/user/projects/diff/delete') }}/${projectId}`,
            type: 'DELETE',
            data: {
                _token: '{{ csrf_token() }}'
            },
 ...
 $('#fundTable').on('click', '.fund', function() {
        ...
        $.ajax({
            url: `{{ url('/user/project/diff/fund/view') }}`,  // Adjust this URL as needed
            type: 'GET',
            data: {
    ...
    $.ajax({
        url: `{{ url("/user/project/diff/fund/new") }}`,  // The URL where the form data will be submitted
        type: 'POST',
        data: formData,
...
$(document).on('click', '.mark', function() {
    ...
    $('#mark').off('click').on('click', function() {
        $.ajax({
            url: `{{ url('/user/project/diff/status/update') }}/${projectId}`,
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}'
            },
...
$(document).on('click', '.paid', function() {
    ...
    // Set the user name and message in the modal
    $('#modalpaidUserName').text(userName);
    $('#modalpaidMessage').text('Are you sure that the payment completed?');

    // Show the modal
    $('#paidConfirmationModal').modal('show');
     $('.close').on('click', function()
    {
        $('#paidConfirmationModal').modal('hide');
    });
    ...
    // Handle confirmation
    $('#paid').off('click').on('click', function() {
        $.ajax({
            url: `{{ url('/user/project/diff/paid/update') }}/${projectId}`,
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}'
            },
            success: function(response) {
                ...
                $('#fundTable').DataTable().ajax.reload();
                $('#paidConfirmationModal').modal('hide'); // Hide the modal on success
                localStorage.setItem('activeTab', '#tab-specs');
            },
            error: function(xhr, status, error) {
                console.error(xhr.responseText);
